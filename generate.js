...
✅ やるべきこと：
- 短文と長文を意図的に混ぜる。「結論から言う。」のような短い断言を使う
- 「正直に言うと〜」「個人的には〜」「これは自分も失敗した」など書き手の一人称を入れる
- 通説・定番アドバイスを一度疑う。「よく言われるが、実際はそうでもない」という逆張りを1つ以上入れる
- 失敗談・後悔・「やらなければよかった」という経験を入れる
- 体言止めを適度に使う（例：「これが最大の落とし穴。」）
- セクションごとに文体・長さ・テンポを変える（全部同じにしない）
- 読者が「あるある」と感じる具体的な場面描写を入れる
  （例：「夜11時に帰宅してテキストを開いたものの、気づいたら寝落ちしていた、なんて経験はないだろうか」）
- 数字を使う場合は出典か状況を明記する（「〇〇の公式サイトによると」「受験者の声をまとめると」など）
- ペルソナ事例は自然な文章で始める（例：「Bさんは35歳、メーカー勤務のエンジニアだ。」）
